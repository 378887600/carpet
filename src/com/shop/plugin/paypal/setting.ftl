...
<title>${message("admin.plugin.paypal.setting")}</title>
...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.jhtml">${message("admin.path.index")}</a> &raquo; ${message("admin.plugin.paypal.setting")}
	</div>
	<form id="inputForm" action="update.jhtml" method="post">
		<table class="input">
			...
			<tr>
				<th>
					<span class="requiredField">*</span>${message("admin.plugin.paypal.partner")}:
				</th>
				<td>
			...
			<tr>
				<th>
					${message("admin.plugin.paypal.currency")}:
				</th>
				<td>
					<select name="currency">
						[#list currencies as currency]
							<option value="${currency}"[#if currency == pluginConfig.attributes['currency']] selected="selected"[/#if]>${message("PaypalPlugin.Currency." + currency)}</option>
						[/#list]
					</select>
				</td>
			</tr>
