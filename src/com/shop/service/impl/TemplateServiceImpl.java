...
import com.shop.Template;
import com.shop.Template.Type;
import com.shop.service.TemplateService;
...
public class TemplateServiceImpl implements TemplateService, ServletContextAware {
	...
	public List<Template> getAll() {
		try {
			...
			List<Element> elements = document.selectNodes("/carpetshop/template");
	...
	public List<Template> getList(Type type) {
		if (type != null) {
			try {
				...
				Document document = new SAXReader().read(shopXmlFile);
				List<Template> templates = new ArrayList<Template>();
				List<Element> elements = document.selectNodes("/carpetshop/template[@type='" + type + "']");
				for (Element element : elements) {
					Template template = getTemplate(element);
					templates.add(template);
				}
	...
	public Template get(String id) {
		try {
			File shopXmlFile = new ClassPathResource(CommonAttributes.SHOP_XML_PATH).getFile();
			Document document = new SAXReader().read(shopXmlFile);
			Element element = (Element) document.selectSingleNode("/carpetshop/template[@id='" + id + "']");
			Template template = getTemplate(element);
			return template;
		} catch (Exception e) {
			e.printStackTrace();
